    ...
    def get_losses(
        ...
        for batch_idx in range(outputs.shape[0]):
            ...
            if num_gt == 0:
                ...
                iou_positive=gt_fg_class_matrix * pred_ious_this_matching.unsqueeze(-1)        # [1,m_fg,1] iou   
                # self.iou_loss(bbox_preds[batch_idx].view(-1, 4)[fg_masks], reg_target)    
                iou_positive_loss=-torch.log(iou_positive+1e-12)       
                loc_conf=torch.exp(-iou_positive_loss*5)               # location quality 
                
                cls_conf=cls_preds[batch_idx].view(-1, self.num_classes)[fg_mask].sigmoid()* obj_preds[batch_idx].view(-1, 1)[fg_mask].sigmoid() 
                joint_conf=loc_conf*cls_conf.unsqueeze(0)       # [n,m_fg,8]       
        ...
        # obj weight 
        obj_pos_weights=torch.ones_like(obj_targets.to(torch.float32))     # [M_fg , 8]
        # reg_pos_weights=  (reg_pos_weights*fg_masks.sum())/(reg_pos_weights.sum())
        obj_loss = (          
           self.bcewithlog_loss(obj_preds.view(-1, 1), obj_targets)  
        ).squeeze(1)   # [H*W]                                                 

        reg_pos_weights=(reg_pos_weights*obj_loss[fg_masks].sum() )/ (reg_pos_weights*obj_loss[fg_masks]).sum()     # [M_fg]

        obj_pos_weights=obj_pos_weights.squeeze(1)     # [M_fg]
        obj_pos_weights[fg_masks]= reg_pos_weights
        obj_pos_weights=obj_pos_weights.unsqueeze(1)
        obj_pos_weights.detach()

        loss_obj = (
            obj_pos_weights*self.bcewithlog_loss(obj_preds.view(-1, 1), obj_targets)
        ).sum() / num_fg
        loss_cls = (
